...
for i in tqdm(range(10)):
    x =  joblib.load(r"F:\eps-9.96.data" + f'-{i}.pkl')
...
for n in range(section*4,section*5):
    ...
    for i in range(100000):
        ...
        #print(label3[i])
        #print(list_inx[n])
        for z in range(len(list_inx[n-section*4][label3[i]])):
            nimi_inx = list_inx[n-section*4][label3[i]][z]
    ...
    votes_inx[n-section*4] = lis
np.savetxt(r"F:\test/votes_inx_mnist_80.16_eps10_fen1_4000-5000.txt", np.asarray(votes_inx), delimiter=",",fmt="%d")
import sys
sys.exit()
