...
import numpy as np
conp = 0
#clean_votes = np.ones((section_num, 100000))
...
for n in range(section,section*2):
    ...
    for i in range(100000):
        ...
        #print(label3[i])
        #print(list_inx[n])
        for z in range(len(list_inx[n-section][label3[i]])):
            nimi_inx = list_inx[n-section][label3[i]][z]
    ...
    votes_inx[n-section] = lis
np.savetxt(r"F:\test/votes_inx_mnist_79.05_eps10_fen2_1000-2000.txt", np.asarray(votes_inx), delimiter=",",fmt="%d")
import sys
sys.exit()
