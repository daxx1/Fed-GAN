...
import numpy as np
conp = 0
#clean_votes = np.ones((num_teacher, 100000))
...
for n in range(section*2,section*3):
    ...
    for i in range(100000):
        ...
        #print(label3[i])
        #print(list_inx[n])
        for z in range(len(list_inx[n-section*2][label3[i]])):
            nimi_inx = list_inx[n-section*2][label3[i]][z]
    ...
    votes_inx[n-section*2] = lis
np.savetxt(r"F:\test/votes_inx_mnist_79.05_eps10_fen2_2000-3000.txt", np.asarray(votes_inx), delimiter=",",fmt="%d")
import sys
sys.exit()
