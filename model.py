...
class DCGAN(object):
    ...
    def aggregate_results(self, output_list, config, thresh=None, epoch=None):
        if self.pca:
            ...
            rres = []
            res, rdp_budget = signsgd_aggregate(output_list[0:1666], config.sigma, self.orders, config.topk,
                                                config.thresh,
                                                alpha=config.learning_rate, stochastic=config.stochastic, b=b)
            rres.append(res)
            res1, rdp_budget1 = signsgd_aggregate(output_list[1666:3332], config.sigma, self.orders, config.topk,
                                                  config.thresh,
                                                  alpha=config.learning_rate, stochastic=config.stochastic, b=b)

            rres.append(res1)
            res2, rdp_budget2 = signsgd_aggregate(output_list[3332:5000], config.sigma, self.orders, config.topk,
                                                  config.thresh,
                                                  alpha=config.learning_rate, stochastic=config.stochastic, b=b)
            rres.append(res2)
            rdp_budget = rdp_budget + rdp_budget1 + rdp_budget2
    ...
    def train_together(self, config):
        ...
        for epoch in xrange(config.epoch):
                ...
                for k in range(config.g_step):
                    ...
                    for j in range(self.batch_size):
                        ...
                        if config.non_private:
                            ...
                            img_grads_agg_tmp, rdp_budget = self.aggregate_results(
                                [grads[j] for grads in img_grads_list], config, thresh=thresh)
                            img_grads_agg_tmp2 = self.non_private_aggregation(img_grads_agg_tmp, config)
                            print('GGGGGGGGGGGGGGGGGGGGGGGGG')
                            print("img_grads_agg_tmp2", img_grads_agg_tmp2)
                            img_grads_agg_tmp = torch.sign(torch.from_numpy(img_grads_agg_tmp2)).numpy()*1e-3
                            print('NNNNNNNNNNNNNNNNNNNNNNNNNNN')
                            print("img_grads_agg_tmp", img_grads_agg_tmp[0])
            ...
            if epoch % 25 == 0:
                ...
                if not os.path.exists(output_dir):
                    os.makedirs(output_dir)
                filename = 'private.data_epoch_' + str(epoch) + '.data'
                outfile = os.path.join(output_dir, filename)
                mkdir(output_dir)
                with open(outfile, 'wb') as f:
                    pickle.dump(data, f)
                ...
                data = data[:100000]
                import joblib
                joblib.dump(data, self.checkpoint_dir + '/eps-%.2f.data' % self.dp_eps_list[-1]+str(epoch))
                #sys.exit()
