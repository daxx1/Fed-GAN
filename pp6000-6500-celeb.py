...
def load_celeb():
    ...
    tst_x = joblib.load(r'F:/162770train32323x.pkl')
    tst_y = joblib.load(r'F:/162770train32323y.pkl')
...
import joblib
#data = joblib.load(r"F:\eps-2.99.data80")

data = np.zeros((100000, 3074))
dim = 0
import joblib

from tqdm import tqdm
for i in tqdm(range(10)):
    x =  joblib.load(r"F:/eps-2.98.data" + f'-{i}.pkl')
...
import cv2
#list_inx = []
list_inx =[[[]for z in range(2)] for p in range(section)]
for n in range(section*12,section*13):

    #if label[i].tolist() == train_label_list[n][l].tolist():
    inx = [np.argmax(i) for i in train_label_list[n]]
    for j in range(len(inx)):
        list_inx[n-section*12][inx[j]].append(j)
for n in range(section*12,section*13):
    lis = []
    ...
    for i in range(100000):
        ...
        #print(label3[i])
        #print(list_inx[n])
        for z in range(len(list_inx[n-section*12][label3[i]])):
            nimi_inx = list_inx[n-section*12][label3[i]][z]
    ...
    votes_inx[n-section*12] = lis
np.savetxt(r"F:\test/votes_inx_celeba69.05_e3_fenbu1_6000-6500.txt", np.asarray(votes_inx), delimiter=",",fmt="%d")
import sys
sys.exit()
